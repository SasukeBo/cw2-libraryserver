...
router.get('/:bookID', function(req, res) {
  if (req.query.allEntities == 'true') {
    db.Book.findByPk(req.params.bookID, { include: [db.Author] }).then(function(
      book
    ) {
      if (book) {
        ret.json(book, res)
...
router.post('/', function(req, res) {
  db.Book.create({ title: req.body.title, isbn: req.body.isbn }).then(function(
    book
  ) {
    ret.json(book, res)
  })
...
router.post('/:bookID/authors', function(req, res) {
  db.Book.findByPk(req.params.bookID, { include: [db.Author] }).then(function(
    ...
    if (book) {
      db.Author.findOrCreate({ where: { name: req.body.name } }).spread(
        function(author, created) {
